...
#ifdef LOGD_DEBUG
#define DEBUG_LOG(fmt, ...) PRINT(fmt, __VA_ARGS__);
#else
...
#define PRINT(fmt, ...)                                                        \
	{                                                                          \
		...
		prop_t __prop[7];                                                      \
		log_init(&__log);                                                      \
		log_set(&__log, &__prop[0], KEY_CLASS, __FILE__);                      \
		log_set(&__log, &__prop[1], "func", __func__);                         \
		log_set(&__log, &__prop[2], "line", STR(__LINE__));                    \
		log_set(&__log, &__prop[3], KEY_LEVEL, "DEBUG");                       \
		log_set(&__log, &__prop[4], KEY_TIME, util_get_time());                \
		log_set(&__log, &__prop[5], KEY_DATE, util_get_date());                \
		int __size = snprintf(NULL, 0, fmt, __VA_ARGS__);                      \
		char* __msg = malloc(__size + 1);                                      \
		sprintf(__msg, fmt, __VA_ARGS__);                                      \
		log_set(&__log, &__prop[6], KEY_MESSAGE, __msg);                       \
